...
def cal_movement_hist(tracked, width1, height1):
    ...
    for ids in tracked:
        ...
        for box in tracked[ids]:
            ...
            total_dist += dist
            first_box = box
        movements.append((5 * pixel2mm * total_dist) / frames)
...
for date in os.listdir(input_video_dir):
    ...
    if not os.path.isdir("/workspace/nas-data/Research_Group/Personal/chenBA/test_video/NNI_k8s/"+ vid_date):
        os.mkdir("/workspace/nas-data/Research_Group/Personal/chenBA/test_video/NNI_k8s/"+ vid_date)
    ...
    for rpi in videos:
        ...
        if not os.path.isdir("/workspace/nas-data/Research_Group/Personal/chenBA/test_video/NNI_k8s/"+ vid_date + "/" + rpi):
            os.mkdir("/workspace/nas-data/Research_Group/Personal/chenBA/test_video/NNI_k8s/"+ vid_date + "/" + rpi)
        ...
        for video in videos[rpi]:
            ...
            while True:
                ...
                # # draw line
                # cv2.line(frame, (0, 50), (1600, 50), (0, 255, 255), 5)
                # cv2.line(frame, (0, 1000), (1600, 1000), (0, 255, 255), 5)
                # # draw counter
                ...
                # cv2.putText(frame, "FPS: " + str(round(1/(end-start))), (50,50), cv2.FONT_HERSHEY_DUPLEX, 2, (0, 255, 255), 10)
            ...
            #---------------------------------------------------------------
            # movement_hist = cal_movement_hist(tracked, width1, height1)
            #
            # histogram = plt.hist(movement_hist, bins='auto')
            # plt.xlim(left=0)
            # plt.title('Histogram of chicken movements')
            # plt.xlabel('Movements (mm/s)')
            # plt.ylabel('Number of chicken')
            # plt.savefig('hist/{}.jpg'.format(os.path.basename(video).split('.')[0]), dpi=300)
            #plt.show()
            #release the file pointers
            dumped = json.dumps(tracked, cls=NumpyEncoder)
            ...
            with open("/workspace/nas-data/Research_Group/Personal/chenBA/test_video/NNI_k8s" + "/" + os.path.basename(date_path) + "/" + rpi + "/" + os.path.basename(video).split('.')[0] + ".csv", 'w', newline='') as f:
                csv_writer = csv.writer(f, delimiter=',') 
                for i, x in enumerate(avg_movements):
                    csv_writer.writerow([5*(i+1), x, std_movements[i], avg_NNI[i], std_NNI[i]])
            vs.release()
            ends = time.time()
            print('video_time: ', ends-starts)
            print('FPS: ', sum(fps)/18000)
