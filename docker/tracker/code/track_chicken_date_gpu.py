...
yolo_dir = "/workspace/tracker/code/yolo"
dt_confidence = 0.15
nms_threshold = 0.3
pixel2mm_1920 = 3.416
pixel2mm_1280 = 3.48
...
for rpi in videos:
    ...
    for video in videos[rpi]:
        ...
        # print(movement)
            ...
            move = {}
            # To avoid nan
            if math.isnan(avg_mov)==True:avg_mov=0
            if math.isnan(std_mov)==True:std_mov=0
            if math.isnan(nni)==True:nni=0
            if math.isnan(std_nni)==True:std_nni=0
            NNI = []
            ends = time.time()
            ...
            # When you need to save .csv or more then 5 minites
